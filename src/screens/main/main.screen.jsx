...
export const MainScreen = () => {
  ...
  const handleInputValueChange = event => {
    ...
    setInputValue(previous => ({ ...previous, [input.name]: input.value }))
  }

  const handleInputCepBlur = async () => {
    const inputCepClean = inputValue.cep.replace(/\D/g, '')
      .substring(0, 8)
      .padEnd(8, '0')

    const inputCepMask = `${inputCepClean.substring(0, 5)}-${inputCepClean.substring(5)}`
    ...
    try {
      const response = await fetch.getCepInfo(inputCepMask)
      ...
      <form className='cep-container'>
        <label className='cep-input-label'>CEP:
          <div className='cep-input-container'>
            <input className={`cep-input ${!cepInfo && 'invalid-input'}`} name='cep' value={inputValue.cep} placeholder='Ex. 93348-320' disabled={isCepLoading} onChange={handleInputValueChange} onBlur={handleInputCepBlur} />
            ...
            {!cepInfo && !isCepLoading && <p className='cep-error'>CEP não encontrado!</p>}
          </div>
        </label>

        <label className='cep-input-label'>Rua:
          <input className='cep-input' name='street' value={inputValue.street} placeholder='Ex. Rua União Sul Africana' disabled={isCepLoading} onChange={handleInputValueChange} />
        </label>

        <label className='cep-input-label'>Bairro:
          <input className='cep-input' name='district' value={inputValue.district} placeholder='Ex. Rincão' disabled={isCepLoading} onChange={handleInputValueChange} />
        </label>

        <div className='cep-city-state-container'>
          <label className='cep-input-label'>Cidade:
            <input className='cep-input' name='city' value={inputValue.city} placeholder='Ex. Novo Hamburgo' disabled={isCepLoading} onChange={handleInputValueChange} />
          </label>

          <label className='cep-input-label'>Estado:
            <input className='cep-input' name='state' value={inputValue.state} placeholder='Ex. RS' disabled={isCepLoading} onChange={handleInputValueChange} />
          </label>
        </div>
      </form>
    </section>
