...
export const MainScreen = () => {
  ...
  useEffect(() => {
    const fetchCepInfo = async () => {
      if (inputValue.cep.length === cepLength) {
  ...
  const handleInputValueChange = async (event) => {
    ...
    const inputMask = input.name === 'cep'
      ? input.value
        .replace(/[\D]/g, '')
        .replace(/([^]{5})([^])/, '$1-$2')
      : input.value

    setInputValue(previous => ({ ...previous, [input.name]: inputMask }))
  }

  const isCepWarning = inputValue.cep.length > 0
    && inputValue.cep.length !== cepLength
    && !isCepLoading

  const isCepError = !cepInfo
    && inputValue.cep.length === cepLength
    && !isCepLoading
      ...
      <form className='cep-container'>
        <label className='cep-input-label'>CEP:
          <div className='cep-input-container'>
            <input className={`cep-input ${isCepError && 'invalid-input'}`} name='cep' value={inputValue.cep} placeholder='Número do CEP' disabled={isCepLoading} onChange={handleInputValueChange} />
            ...
            {isCepWarning && <p className='cep-warning'>O CEP deve possuir 8 números.</p>}

            {isCepError && <p className='cep-error'>CEP não encontrado!</p>}
          </div>
        </label>
